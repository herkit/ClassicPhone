...
namespace ClassicPhone
{
	...
	public class UpdateService : Service
	{
        ...
        public RemoteViews buildUpdate(Context context)
        {
            ...
            var bmp = Bitmap.CreateBitmap(ota.Width * 3, ota.Height * 3, Bitmap.Config.Alpha8);
            var cnv = new Canvas(bmp);
            var white = new Paint(PaintFlags.AntiAlias) { Color = Color.Red };
            ...
            for (var x = 0; x < ota.Width; x++) {
                for (var y = 0; y < ota.Height; y++) {
                    if (ota.Bits[x, y])
                        cnv.DrawRect(new Rect(x * 3, y * 3, x * 3 + 2, y * 3 + 2), white);
                }
            }
